...
public class JRSandsOfAmberRightClickedInAirProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof Player _player) {
			ItemStack _stktoremove = new ItemStack(FartsysmagitechfantasyModItems.JR_SANDS_OF_AMBER.get());
			_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
		}
		if (entity instanceof Player _player) {
			ItemStack _setstack = new ItemStack(FartsysmagitechfantasyModItems.AMD_SANDS_OF_AMBER.get()).copy();
