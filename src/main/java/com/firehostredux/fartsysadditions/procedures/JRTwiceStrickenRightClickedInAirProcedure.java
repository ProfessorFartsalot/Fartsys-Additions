...
public class JRTwiceStrickenRightClickedInAirProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof Player _player) {
			ItemStack _stktoremove = new ItemStack(FartsysmagitechfantasyModItems.JR_TWICE_STRICKEN.get());
			_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
		}
		if (entity instanceof Player _player) {
			ItemStack _setstack = new ItemStack(FartsysmagitechfantasyModItems.AMD_TWICE_STRICKEN.get()).copy();
