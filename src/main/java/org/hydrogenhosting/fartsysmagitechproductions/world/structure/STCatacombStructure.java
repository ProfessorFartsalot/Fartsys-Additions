...
import org.hydrogenhosting.fartsysmagitechproductions.procedures.STCatacombAdditionalGenerationConditionProcedure;
...
public class STCatacombStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 100000) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!STCatacombAdditionalGenerationConditionProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("y", y))
									.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
								continue;
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("fartsysmagitechproductions", "catacomb"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("st_catacomb"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("fartsysmagitechproductions:st_catacomb"),
					configuredFeature);
		}
	}
